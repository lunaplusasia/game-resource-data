# LUNA Online — GitBook Content

> ข้อมูลสำหรับผู้เล่น LUNA Plus ASIA — สร้างอัตโนมัติจากระบบวิเคราะห์
> Version 2.9.0 | Game Patch 2.0.0

---

## 🔗 Quick Links

| หน้า | คำอธิบาย |
|------|----------|
| [📊 Skill Analysis](skill_analysis/README.md) | ระบบวิเคราะห์สกิลทุกอาชีพ |
| [📊 ภาพรวม](skill_analysis/overview.md) | สรุปข้อมูลสกิลทุกสาย |
| [⚖️ Balance](skill_analysis/balance.md) | ความสมดุลอาชีพ 7 มิติ |
| [📋 สารบัญ](skill_analysis/SUMMARY.md) | สารบัญทั้งหมด |

---

## 📚 Sections

### 📊 Skill Analysis

| รายการ | ข้อมูล |
|--------|--------|
| สถานะ | ✅ พร้อมใช้งาน |
| โฟลเดอร์ | `skill_analysis/` |
| ไฟล์ทั้งหมด | 190 |

**เนื้อหา:**

- ⚔️ **Skills** — 95 อาชีพ (Devil, Fighter, Mage, Rogue)
- 🗡️ **Combat Analysis** — 33 ไฟล์ (CC Chain, DPS Rotation, Matchup)
- 🌳 **Progression** — 4 ไฟล์ (สกิลสะสม C1–C5, Ranking)
- 📈 **Simulation** — 31 ไฟล์ (DPS/HPS, Gear Profiles, Self-Buff)
- 📋 **Consolidated Reports** — 5 ไฟล์ (Overview, Class Guide, Balance, Combat)
- 📄 **หน้าหลัก** — overview.md, balance.md, SUMMARY.md, README.md

---

## 🤖 AI Reviews

วิเคราะห์เชิงลึกโดย AI — อ่านตามลำดับ 01→06:

| # | Model | หัวข้อ | Link |
|---|-------|--------|------|
| 01 | claude | Skill Inventory | [อ่าน](skill_analysis/reviews/claude/01_ai_review_skill_inventory.md) |
| 02 | claude | Progression Analysis | [อ่าน](skill_analysis/reviews/claude/02_ai_review_skill_progression.md) |
| 03 | claude | Combat Analysis | [อ่าน](skill_analysis/reviews/claude/03_ai_review_skill_combat.md) |
| 04 | claude | Simulation Analysis | [อ่าน](skill_analysis/reviews/claude/04_ai_review_skill_simulation.md) |
| 05 | claude | Balance Analysis | [อ่าน](skill_analysis/reviews/claude/05_ai_review_skill_balance.md) |
| 06 | claude | Overview | [อ่าน](skill_analysis/reviews/claude/06_ai_review_skill_overview.md) |
| 01 | gemini | Skill Inventory | [อ่าน](skill_analysis/reviews/gemini/01_ai_review_skill_inventory.md) |
| 02 | gemini | Progression Analysis | [อ่าน](skill_analysis/reviews/gemini/02_ai_review_skill_progression.md) |
| 03 | gemini | Combat Analysis | [อ่าน](skill_analysis/reviews/gemini/03_ai_review_skill_combat.md) |
| 04 | gemini | Simulation Analysis | [อ่าน](skill_analysis/reviews/gemini/04_ai_review_skill_simulation.md) |
| 05 | gemini | Balance Analysis | [อ่าน](skill_analysis/reviews/gemini/05_ai_review_skill_balance.md) |
| 06 | gemini | Overview | [อ่าน](skill_analysis/reviews/gemini/06_ai_review_skill_overview.md) |
| 01 | gpt | Skill Inventory | [อ่าน](skill_analysis/reviews/gpt/01_ai_review_skill_inventory.md) |
| 02 | gpt | Progression Analysis | [อ่าน](skill_analysis/reviews/gpt/02_ai_review_skill_progression.md) |
| 03 | gpt | Combat Analysis | [อ่าน](skill_analysis/reviews/gpt/03_ai_review_skill_combat.md) |
| 04 | gpt | Simulation Analysis | [อ่าน](skill_analysis/reviews/gpt/04_ai_review_skill_simulation.md) |
| 05 | gpt | Balance Analysis | [อ่าน](skill_analysis/reviews/gpt/05_ai_review_skill_balance.md) |
| 06 | gpt | Overview | [อ่าน](skill_analysis/reviews/gpt/06_ai_review_skill_overview.md) |

---

## 📋 Consolidated Reports

| รายงาน | Link |
|--------|------|
| 📊 Report: สรุปรวม (Overview) | [อ่าน](skill_analysis/reports/report_overview.md) |
| 🎯 Report: คู่มือสายอาชีพ (Class Guide) | [อ่าน](skill_analysis/reports/report_class_guide.md) |
| ⚖️ Report: วิเคราะห์ Balance | [อ่าน](skill_analysis/reports/report_balance.md) |
| 📜 Report: Skill Get List | [อ่าน](skill_analysis/reports/report_skill_get_list.md) |
| ⚔️ Report: Combat Summary | [อ่าน](skill_analysis/reports/report_combat.md) |

---

## สรุป

| รายการ | จำนวน |
|--------|------:|
| Sections ที่พร้อมใช้งาน | 1 |
| ไฟล์ Markdown ทั้งหมด | 190 |
| AI Review Files | 18 |

---

**Skill Analysis:** v2.9.0 | **Patch:** v2.0.0

*อัปเดตล่าสุด: 2026-02-14 20:33:25*

📌 **Luna Plus ASIA** | Patch v2.0.0 ― Skill Analysis v2.9.0